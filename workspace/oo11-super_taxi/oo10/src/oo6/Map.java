...
public class Map {
	 ...
	 * repOK<==>!(map[i][j].up==false&&map[i][j].down==false&&map[i][j].left==false&&map[i][j].right==false)==ture;
	 * 
	 * */
	boolean up=false;
	boolean down=false;
	...
	static Map[][] newmap = new Map[maxofmap][maxofmap];

	public boolean repOK(){
		/* @Requires:none;
		 * @Modifies:none;
		 * @Effects:check map 
		 * 			if map is right return true 
		 *  */
		for(int i=0;i<Map.maxofmap;i++){
			for(int j=0;j<Map.maxofmap;j++){
				if(map[i][j].up==false&&map[i][j].down==false&&map[i][j].left==false&&map[i][j].right==false){
					return false;
				}
			}
		}
	...
	public static synchronized void deletepath(Point1 a,Point1 b){
		...
		TaxiGUI aa= new TaxiGUI();
		 if(Point1.isUP(a, b)){
			 ...
			 aa.SetRoadStatus(new Point(a.X,a.Y), new Point(b.X,b.Y), 0);
		 }
	...
	public static synchronized void addpath(Point1 a,Point1 b){
		...
		TaxiGUI aa = new TaxiGUI();
		 if(Point1.isUP(a, b)){
			 ...
			 aa.SetRoadStatus(new Point(a.X,a.Y), new Point(b.X,b.Y), 1);
		 }
		 else if(Point1.isDOWN(a, b)){
	...
	public static void InitDirection(){
		...
		for(int i=0;i<maxofmap;i++){
			for(int j=0;j<maxofmap;j++){
				...
				newmap[i][j]=new Map(false,false,false,false);
			}
		}
	}
                ...
                while((lineTxt = bufferedReader.readLine()) != null){
                	...
                	for(int i=0;i<linesplit.length;i++){
                		if(Integer.parseInt(linesplit[i])==1){
                			...
                			newmap[m][i].right=true;
                			newmap[m][i+1].left=true;
                		}
                		else if(Integer.parseInt(linesplit[i])==2){
                			map[m][i].down=true;
                			map[m+1][i].up=true;
                			
                			newmap[m][i].down=true;
                			newmap[m+1][i].up=true;
                		}
	...
	public synchronized ArrayList<Point1> shortway(Map[][] map,Point1 start,Point1 end){
