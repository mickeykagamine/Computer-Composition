...
public class Taxi extends Thread {	
	...
	Map[][] map =new Map[Map.maxofmap][Map.maxofmap];
//	static int[] Sign=new int[Taxi.taxinumber];
	int sign=0;
	...
	public void waitinglight(String newdir,String olddir,int X,int Y){
		/* @Requires:String newdir,olddir,int X,Y;
		 * @Modifies:none;	
	...
	public static void InitTaxiMap(TaxiGUI gui){
		 ...
		 * */
		int[] sign=new int[Taxi.taxinumber];
		try {
            String encoding="GBK";
            File file=new File("E:\\taxi.txt");
            if(file.isFile() && file.exists()){ //判断文件是否存在
                ...
                while((lineTxt = bufferedReader.readLine()) != null){
                	...
                	for(int i=0;i<linesplit.length;i++){
                		sign[i]=Integer.parseInt(linesplit[i]);
  //              		Sign[i]=sign[i];
                	}
                	m++;
                }
            	...
            	System.out.println("can't find taximap");
            }
		}catch (Exception e) {
	...
	//		System.out.println("taxi"+i+"START AT("+r1+","+r2+")");
			if(sign[i]==0){
				TAXILIST[i]=new Taxi(i,0,"waiting",new Point1(r1,r2),gui,Map.map,sign[i]);	
				gui.SetTaxiType(i,0);
			}
			else if(sign[i]==1){
				TAXILIST[i]=new NewTaxi(i,0,"waiting",new Point1(r1,r2),gui,Map.newmap,sign[i]);	
				gui.SetTaxiType(i,1);
			}
		/*	
			if(i==0){
	...
	public static Taxi[] InitTaxiMap2(){
		/* @Requires:NONE;
		 * @Modifies:TAXILIST[i];	
		 * @Effects:INIT TAXILIST;
		 * */
		return TAXILIST;
	}
	...
	public Taxi(int i, int j,String a,Point1 A,TaxiGUI GUI,Map[][] Map,int Sign) {
