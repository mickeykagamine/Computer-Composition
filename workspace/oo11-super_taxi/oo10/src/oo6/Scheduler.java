...
public class Scheduler extends Thread{
	...
	Point1 B;
	long T;
	Taxi taxinull=new Taxi(-1,-1,"null",new Point1(-1,-1),null,Map.map,0);
	...
	//		System.out.println("chosinglist is not empty!");
		...
		//	System.out.println("count:"+count);
			...
			//	System.out.println("yeaddddd");
				...
				ArrayList<Point1> l =new ArrayList<Point1>();
				if(Taxi.TAXILIST[chosinglist.get(0).num].sign==0){
					l=(ArrayList<Point1>) p.shortway(Map.map,chosinglist.get(0).spot, A).clone();
				}
				if(Taxi.TAXILIST[chosinglist.get(0).num].sign==1){
					l=(ArrayList<Point1>) p.shortway(Map.newmap,chosinglist.get(0).spot, A).clone();
				}
				if(l!=null){
					min=l.size();
				}
				...
				if(l!=null){
					for(int i=0;i<n;i++){
						ArrayList<Point1> l2 =new ArrayList<Point1>();
						if(Taxi.TAXILIST[chosinglist.get(i).num].sign==0){
							l2=(ArrayList<Point1>) p.shortway(Map.map,chosinglist.get(i).spot, A).clone();
						}
						if(Taxi.TAXILIST[chosinglist.get(i).num].sign==1){
							l2=(ArrayList<Point1>) p.shortway(Map.newmap,chosinglist.get(i).spot, A).clone();
						}
