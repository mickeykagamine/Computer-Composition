...
class NewElevator{
	...
	public static boolean repOK(){
		 ...
		 * @Require:none;
		 * @Modified:none;
		 * @Effects:\result==!((floor<=0||floor>10)|(headingfloor<0||headingfloor>10)|(!(!(direction.equals("UP")|(direction.equals("DOWN")|(direction.equals("STILL")|(direction.equals("NULL")))))))));
		 * */
		if(floor<=0||floor>10){
			return false;
		}
		...
		if(!(direction.equals("UP")|(direction.equals("DOWN")|(direction.equals("STILL")|(direction.equals("NULL")))))){
			return false;
		}
		return true;
	}
	static int run(String[][] Queue){
		 ...
		 * @Effects:move the elevator according to the requires;
					if(FR&!running&UP) move the elevator up;
					if(FR&!running&DOWN)move the elevator down;
					if(FR&!running&STILL) stop the elevator down;
					if(FR&running&UP&canpick) add the elevator list;
					if(FR&running&UP&!canpick)do nothing;
					if(FR&running&DOWN&canpick) add the elevator list;
					if(FR&running&DOWN&!canpick)do nothing;
					if(ER&!running&UP) move the elevator up;
					if(ER&!running&DOWN)move the elevator down;
					if(ER&!running&STILL) stop the elevator down;
					if(ER&running&UP&canpick) add the elevator list;
					if(ER&running&UP&!canpick)do nothing;
					if(ER&running&DOWN&canpick) add the elevator list;
					if(ER&running&DOWN&!canpick)do nothing;
					if(DOWN&have require) floor--,stop the elevator;
					if(UP&have require) floor++,stop the elevator;
					if(DOWN&!have require) floor--;
					if(UP&!have require) floor++;
								...
								if(getFloor() < Double.parseDouble(Queue[m][1])&&Double.parseDouble(Queue[m][1]) <= headingfloor){
									for(int n = 999;n >= 0;n--){			
											...
											if(Queue[m][0].equals(Waitinglist[n][0])&&Queue[m][1].equals(Waitinglist[n][1])&&Queue[m][2].equals(Waitinglist[n][2])){
												if( Floor.Sum(n,Waitinglist)>=Double.parseDouble(Queue[m][3])){
													sign = false ;
								...
								if(getFloor() < Double.parseDouble(Queue[m][1])){
									...
									for(int n = 999;n >= 0;n--){			
											...
											if(Queue[m][0].equals(Waitinglist[n][0])
												...
												if( Floor.Sum(n,Waitinglist)>=Double.parseDouble(Queue[m][2])){
													sign = false;
