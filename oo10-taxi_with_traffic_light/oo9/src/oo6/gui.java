...
class guigv {
	...
	public static boolean drawbelief = false;
}
...
class myform extends JFrame {// 我的窗体程序
	...
	public myform() {
		...
		/* 设置复选框属性 */
		JCheckBox check2 = new JCheckBox("显示信用");
		check2.setBounds(450, 520, 200, 40);
		check2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				guigv.drawbelief = check2.isSelected();
				guigv.drawboard.repaint();
			}
		});
	...
	public static void draw(Graphics2D g) {
		...
		for (int i = 0; i < 80; i++) {
			for (int j = 0; j < 80; j++) {
				...
				int targetWidth;
				if (taximap[i][j] == 3) {
					g.setColor(Color.GREEN);//服务状态
					targetWidth = 2;
				} else if (taximap[i][j] == 2) {
					g.setColor(Color.RED);//随机游走状态
					targetWidth = 2;
				} else if (taximap[i][j] == 1) {
					g.setColor(Color.BLUE);//停止状态
					targetWidth = 2;
				} else if (taximap[i][j] == 0) {
					g.setColor(Color.YELLOW);//接单状态
					targetWidth = 2;
				} else {
					g.setColor(Color.BLACK);
					targetWidth = 1;
				...
				if (guigv.drawstr == true) {
					int taxiInMap = taximap[i][j];
					int belief = 0;
					if(taxiInMap>=0){
						//就说明这个地方有车，把车的index找出来
						int taxiIndex =-1;
						int i1=0;
						for(i1=0;i1<guigv.taxilist.size();i1++){
							if(guigv.taxilist.get(i1).x==i&&guigv.taxilist.get(i1).y==j){
								taxiIndex = i1;
								break;
							}
						}
						//找到对应的信用值
					    belief = guigv.taxilist.get(taxiIndex).belif;
					    g.setColor(Color.WHITE);
						g.setFont(new Font("Arial", Font.PLAIN, (int) (8 * guigv.percent)));
						g.drawString("" + i + "," + j+" "+belief, 
								(int) ((j * factor + xoffset + guigv.xoffset-6) * guigv.percent),
...
class TaxiGUI {// GUI接口类
    ...
    //把它输出到一个文件中
	public boolean writeTaxiInformToFile(String path){
	
    	try {
			FileWriter fileWrite = new FileWriter(path);
			for(int i=0;i<guigv.taxilist.size();i++){
				int x,y,belief,status;
				x = guigv.taxilist.get(i).x;
				y = guigv.taxilist.get(i).y;
				belief = guigv.taxilist.get(i).belif;
				status = guigv.taxilist.get(i).status;
				
				fileWrite.write(x+" , "+y+" , "+belief+" , " +status+" ");
				fileWrite.write("\r\n");
	...
	public void MySetTaxiStatus(Point point, int status,int belief) {
		int index = 0;
		for(int i=0;i<guigv.taxilist.size();i++){
			if(guigv.taxilist.get(i).x==point.x&&guigv.taxilist.get(i).y==point.y){
				index = i;
				break;
			}
		}
	...
	public void MySetTaxiLocal(Point sp, Point dp) {
		// TODO Auto-generated method stub
		int index = 0;
		for(int i=0;i<guigv.taxilist.size();i++){
			if(guigv.taxilist.get(i).x==sp.x&&guigv.taxilist.get(i).y==sp.y){
				index = i;
				break;
			}
		}
